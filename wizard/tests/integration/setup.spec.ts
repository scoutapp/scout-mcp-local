...
describe('setup - integration test', () => {
  ...
  it('should complete full setup flow with multiple orgs and Claude Code selection', async () => {
    ...
    expect(vi.mocked(open).mock.calls[0][0]).toMatch(/^https:\/\/test\.scoutapm\.com\/uat\/auth\/sign_in\/[A-Za-z0-9_-]+$/)
    ...
    expect(mockExecAsync).toHaveBeenCalledWith('claude mcp add scout-apm -e SCOUT_API_KEY=claude-code-api-key -- docker run --rm -i -e SCOUT_API_KEY scoutapp/scout-mcp-local:latest')
  })
})
